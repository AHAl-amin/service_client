...
export default function SellerPricing() {
  ...
  const handleGetStarted = async (planId) => {
    ...
    try {
      const response = await SubscribtionPlan({ plan_id: planId }).unwrap();
      console.log("Checkout session created:", response);
      // Assuming the backend returns a checkout session URL or ID
      if (response.url) {
        window.location.href = response.url; // Redirect to checkout session (e.g., Stripe)
      } else {
        navigate("/subscription-success"); // Or handle success differently
      }
    } catch (err) {
