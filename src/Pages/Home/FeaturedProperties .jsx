...
//             <button
//               type="submit"
//               className="h-12 px-8 bg-[#1C3988] placeholder-[#1c398869] text-white rounded-md flex items-center justify-center hover:bg-blue-900 transition duration-300"
//             >
...
export const FeaturedProperties = ({ onExploreClick }) => {
  ...
  const onSubmit = (data) => {
    const filtered = properties
      .filter((property) => {
        ...
        // Filter by down payment availability
        const matchesDownPayment = data.hasDownPayment
          ? property.allow_down_payment
          : true;

        // Filter by down payment amount (if provided)
        const matchesDownPaymentAmount = data.downPayment
          ? parseFloat(property.price) * (property.down_payment_percentage / 100) >=
  ...
  // Initialize filtered properties on component mount or when properties change
  useEffect(() => {
