...
import { useChangePasswordMutation, useGetBuyerDataProfileQuery, useUpdateBuyerDataProfileMutation,  } from "../../redux/features/profileApi";
...
export default function SellerSetting() {
  const { data: getBuyerDataProfile, isLoading, isError, error, refetch } = useGetBuyerDataProfileQuery();
  const profile = getBuyerDataProfile?.data;
  const [activeView, setActiveView] = useState("editProfile");
  const [country, setCountry] = useState("");
  const [updateBuyerDataProfile] = useUpdateBuyerDataProfileMutation();
  ...
  const EditProfileForm = () => {
    const onSubmit = async (data) => {
      ...
      try {
        ...
        formDataToSubmit.append("company_name", data.company_name);
        ...
        const response = await updateBuyerDataProfile(formDataToSubmit).unwrap();
        ...
        if (response?.profile_picture) {
          setPreviewImage(`$https://yoursafeland.duckdns.org${response.profile_picture}`);
        }
      } catch (err) {
